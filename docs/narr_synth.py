#!/usr/bin/env python3
"""Stage 1 of the narrated cut: synthesize per-slide narration in the cloned
voice (XTTS, sentence-chunked so there's no warm-up garble) and master each
slide with the approved 'clear + sharp' chain. Run with the tts-venv python.

Outputs: docs/video_build/narr/slide_vo_NN.wav (48k, mastered) + durations.json
"""
...
for i, chunks in enumerate(NARR):
    ...
    for j, txt in enumerate(chunks):
        ...
        tts.tts_to_file(text=txt, speaker_wav=REF, language="en", file_path=f)
        cfiles.append(f)
